...
void CameraState::run(PubMaster *pm, const char *pub_name) {
  util::set_thread_name(pub_name);

  // Get the member function pointer for initializing the camera state based on the stream type
  auto initCameraState = [type = buf.stream_type]() {
    if (type == VISION_STREAM_DRIVER) return &cereal::Event::Builder::initDriverCameraState;
    if (type == VISION_STREAM_ROAD) return &cereal::Event::Builder::initRoadCameraState;
    return &cereal::Event::Builder::initWideRoadCameraState; }();
  ...
  for (uint32_t cnt = 0; !do_exit; ++cnt) {
    ...
    MessageBuilder msg;
    auto framed = (msg.initEvent().*initCameraState)();
    fill_frame_data(framed, buf.cur_frame_data, this);

    // Log raw frames for road camera
    if (env_log_raw_frames && buf.stream_type == VISION_STREAM_ROAD && cnt % 100 == 5) {  // no overlap with qlog decimation
      framed.setImage(get_raw_frame_image(&buf));
    }
    // Log frame id for road and wide road cameras
    if (buf.stream_type != VISION_STREAM_DRIVER) {
      LOGT(buf.cur_frame_data.frame_id, "%s: Image set", pub_name);
    }

    // Process camera registers and set camera exposure
    ci->processRegisters(this, framed);
    set_camera_exposure(set_exposure_target(&buf, ae_xywh, 2, buf.stream_type != VISION_STREAM_DRIVER ? 2 : 4));

    // Send the message
    pm->send(pub_name, msg);
    if (buf.stream_type == VISION_STREAM_ROAD && cnt % 100 == 3) {
...
void cameras_run(MultiCameraState *s) {
  ...
  if (s->driver_cam.enabled) threads.emplace_back(&CameraState::run, &s->driver_cam, s->pm, "driverCameraState");
  if (s->road_cam.enabled) threads.emplace_back(&CameraState::run, &s->road_cam, s->pm, "roadCameraState");
  if (s->wide_road_cam.enabled) threads.emplace_back(&CameraState::run, &s->wide_road_cam, s->pm, "wideRoadCameraState");
